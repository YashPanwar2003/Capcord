...
export const getAllVideos = withErrorHandling(async (searchQuery: string,
    ...
    const visibilityCondition = or(eq(videos.visibility, "public"), eq(videos.userId, currentUserId!))
    const whereCondition = searchQuery.trim() ? and(visibilityCondition, doesTitleMatch(videos, searchQuery),) : visibilityCondition;
    const [{ totalCount }] = await db.select({ totalCount: sql<number>`count(*)` }).from(videos).where(whereCondition)
    const totalVideos = Number(totalCount || 0)
    const totalPages = Math.ceil(totalVideos/pageSize)
    const videoRecords = await buildVideoWithUserQuery().where(
        whereCondition
